 ...
 *
 */
public class ClusterRouteMergeDFS implements ConstructionAlgorithm {
    DataModel dataModel;
    static final int NUM_CLUSTER_THRESHOLD = 20;
    static final int DELTA_THRESHOLD = 4;
    ...
    public List<Route> run(DataModel dataModel) {
        ...
        for (int numClusters = 1; numClusters <= NUM_CLUSTER_THRESHOLD; numClusters++) {
...
//        logger.info("DFS solution, # vehicles: " + (bestSolution == null ? "-1" : bestSolution.size()));
     ...
     *
     * Here we could select m departure times out of all possible departure times
     * (based on some mechanism) to save runtime.
     *
    ...
    List<Route> runVehicleNumberReduction(List<Route> solution) {
        ...
        List<Route> shortRoutes = solution.stream().filter(r -> r.getNumDemandNodes() < DELTA_THRESHOLD).collect(Collectors.toList());
        ...
        if (newSolution.size() > solution.size()) return solution;  // worsening move, discard
            ...
            List<Route> newShortRoutes = newSolution.stream().filter(r -> r.getNumDemandNodes() < DELTA_THRESHOLD).collect(Collectors.toList());
